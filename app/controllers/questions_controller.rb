class QuestionsController < ApplicationController
  ...
  def edit
    ...
    if is_owner_of @post2edit or user_admin?
      ...
      redirect_to "/questions/read", alert: "이 게시물의 작성자가 아닙니다"
    elsif params[:id].nil?
      redirect_to "/questions/read/#{params[:page_id]}", alert: "이 게시물의 작성자가 아닙니다"
    else
      redirect_to "/questions/read/#{params[:page_id]}/#{params[:id]}", alert: "이 게시물의 작성자가 아닙니다"
    end
  end
  ...
  def create
    ...
    post2create.save
    redirect_to "/questions/read/0/#{post2create.id}"
  end
  ...
  def update
    unless params[:id].nil?
      ...
      if is_owner_of post2update or user_admin?
        ...
        post2update.save
        redirect_to "/questions/read/#{params[:page_id]}/#{params[:id]}", notice: "글을 수정하였습니다"  
      else
        redirect_to "/questions/read/#{params[:page_id]}/#{params[:id]}", alert: "이 게시물의 작성자가 아닙니다"
      end
    else
      redirect_to "/questions/read/#{params[:page_id]}/#{params[:id]}", alert: "잘못된 접근입니다"
    end
  end
  ...
  def delete
    if params[:id].present?
      ...
      if is_owner_of post2delete or user_admin?
        post2delete.destroy
        redirect_to "/questions", notice: "Successfully deleted"
      elsif post2delete.nil?
        redirect_to "/questions", alert: "No such post exists"
      else
        redirect_to "/questions/read/#{params[:page_id]}/#{post2delete.id}", alert: "Not Authorized"
      end
    else
      redirect_to "/questions", alert: "Failed to delete post : No such ID exists"
    end
  end
