class ReplyQuestionsController < ApplicationController
  ...
  def create
    ...
    unless session[:user_id].nil? or params[:id].nil?
      ...
      reply2create.save
      redirect_to "/pp2015/questions/read/" + pagenum.to_s + "/#{params[:id]}", notice: "댓글이 작성되었습니다"
    else
      redirect_to "/pp2015/questions/read/" + pagenum.to_s + "/#{params[:id]}", alert: "댓글을 등록할 수 없습니다"
    end
  end
  ...
  def delete
    ...
    if reply.present?
      if session[:user_id] == reply.user.id
        ...
        redirect_to "/pp2015/questions/read/" + pagenum.to_s + "/" + post_id.to_s, notice: "댓글이 삭제되었습니다"
      else
        redirect_to "/pp2015/questions/read/" + pagenum.to_s + "/" + post_id.to_s, alert: "댓글을 삭제 할 수 없습니다"
      end
    else
        redirect_to "/pp2015/"
    end
